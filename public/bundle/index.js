...
${frame.code}`;
      ...
      <div>
        ...
        ${
          diagnostic.documentation
            ? `<div>\u{1F4DD} <a style="color: violet" href="${
                diagnostic.documentation
    ...
    f2QDv: [
      function(require, module, exports) {
        ...
        const logoutBtnEle = document.querySelector(
          '.nav__el--logout'
        );
        ...
        if (loginFormEle)
          loginFormEle.addEventListener('submit', e => {
            ...
            (0, _authJs.loginFn)(email, password);
          });
        if (logoutBtnEle)
          logoutBtnEle.addEventListener(
            'click',
            (0, _authJs.logoutFn)
          );
        if (updateUserDataFormEle)
          updateUserDataFormEle.addEventListener(
            'submit',
            (0, _authJs.updateUserDataFn)
          );
        if (updatePasswordFormEle)
          updatePasswordFormEle.addEventListener(
            'submit',
            (0, _authJs.updatePasswordFn)
          );
        if (bookTourBtnEle)
          bookTourBtnEle.addEventListener(
            'click',
            (0, _stripe.bookTourFn)
          );
      },
      { './auth.js': 'fov0Z', './stripe': '10tSC' },
    ],
    fov0Z: [
      function(require, module, exports) {
        ...
        parcelHelpers.export(exports, 'logoutFn', () => logoutFn);
        ...
        const loginFn = async (email, password) => {
          try {
            ...
            if (!res.ok) {
              ...
              throw new Error(
                data.message || 'Error logging in! Try again'
              );
            }
            const data = await res.json();
            (0, _alertJs.showAlert)(
              'success',
        ...
        const logoutFn = async e => {
          ...
          try {
            ...
            if (!res.ok) {
              const data = await res.json();
              throw new Error(
                data.message || 'Error logging out! Try again'
              );
            }
        ...
        const updatePasswordFn = async e => {
          e.preventDefault();
          const passwordEle = document.getElementById(
            'password-current'
          );
    ...
    '10tSC': [
      function(require, module, exports) {
        var parcelHelpers = require('@parcel/transformer-js/src/esmodule-helpers.js');
        parcelHelpers.defineInteropFlag(exports);
        parcelHelpers.export(exports, 'bookTourFn', () => bookTourFn);
        var _alertJs = require('./alert.js');
        const bookTourFn = async e => {
          try {
